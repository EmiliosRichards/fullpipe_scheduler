...
from .scraper_utils import normalize_url, get_safe_filename, extract_text_from_html, find_links, _classify_page_type, validate_link_status
...
async def _perform_scrape_for_entry_point(
    ...
    try:
        while urls_to_scrape_q:
            ...
            html_content, status_code_fetch = await fetch_page_content(
                page, current_url_from_queue, input_row_id, company_name_or_id, navigation_steps
            )
            ...
            if html_content:
                ...
                if current_depth < config_instance.max_depth_internal_links:
                    newly_found_links_with_scores = find_links(html_content, final_landed_url_normalized, input_row_id, company_name_or_id)
        ...
        try:
            ...
            while not entry_candidates_queue.empty():
                ...
                details, status, canonical_landed, collected_summary_text = await _perform_scrape_for_entry_point(
                    current_entry_url_to_attempt, playwright_context, http_client_for_validation, output_dir_for_run,
