...
from src.core.schemas import PhoneNumberLLMOutput
...
def create_error_llm_item(
    input_item_details: Dict[str, Any],
    error_type_str: str,
    classification_str: str = "Non-Business", # Default as per original
    file_identifier_prefix: Optional[str] = "N/A_HELPER",
    triggering_input_row_id: Optional[Any] = "N/A_HELPER",
    triggering_company_name: Optional[str] = "N/A_HELPER"
) -> PhoneNumberLLMOutput:
    """
    Creates a PhoneNumberLLMOutput object populated with error information.
    This is typically used when LLM processing fails for an item.

    Args:
        input_item_details (Dict[str, Any]): Dictionary containing details of the input item
                                             that failed. Expected keys: 'number', 'source_url',
                                             'original_input_company_name'.
        error_type_str (str): A string indicating the type of error.
        classification_str (str, optional): Classification for the item, defaults to "Non-Business".
        file_identifier_prefix (Optional[str], optional): Prefix for logging context.
        triggering_input_row_id (Optional[Any], optional): Row ID for logging context.
        triggering_company_name (Optional[str], optional): Company name for logging context.

    Returns:
        PhoneNumberLLMOutput: An instance of PhoneNumberLLMOutput representing the error.
    """
    original_number = input_item_details.get('number', 'Unknown Number')
    source_url = input_item_details.get('source_url', 'Unknown Source')
    
    logger.warning(
        f"[{file_identifier_prefix}, RowID: {triggering_input_row_id}, Company: {triggering_company_name}] "
        f"Creating error item for input number '{original_number}' from source '{source_url}' due to: {error_type_str}"
    )
    return PhoneNumberLLMOutput(
        number=str(original_number),  # Ensure number is a string
        type=error_type_str,
        classification=classification_str,
        is_valid=False,  # Error items are not considered valid
        source_url=str(source_url),
        original_input_company_name=input_item_details.get('original_input_company_name')
    )

def process_successful_llm_item(
    ...
    """
    Enriches a successfully parsed LLM output (PhoneNumberLLMOutput) with details
    from the input item and normalizes the phone number.

    Args:
        llm_output (PhoneNumberLLMOutput): The Pydantic model instance parsed from LLM response.
        input_item_details (Dict[str, Any]): Dictionary with original input details.
                                             Expected keys: 'source_url', 'original_input_company_name', 'number' (original).
        target_country_codes (List[str]): List of country codes for phone normalization.
        default_region_code (Optional[str]): Default region code for phone normalization.

    Returns:
        PhoneNumberLLMOutput: The enriched and (potentially) normalized llm_output.
    """
    ...
    if llm_output.number:
        ...
        if normalized_num:
            ...
            original_input_number = input_item_details.get('number', 'N/A')
            logger.warning(
                f"LLM output number '{llm_output.number}' (from input '{original_input_number}') "
                f"could not be normalized with countries {target_country_codes} and default region '{default_region_code}'. Keeping as is."
            )
    return llm_output
